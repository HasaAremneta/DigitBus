      ...
      <div class="grid">
        ...
        <section class="selector">
          ...
          <div class="cards">
            ...
            <button
              class="card"
              :class="{ active: tipoSeleccionado==='tercera', disabled: accion==='renovacion' }"
            ...
            <button
              class="card"
              :class="{ active: tipoSeleccionado==='general', disabled: accion==='renovacion' }"
...
.logo { height: 32px; }
.top-nav nav a {
  margin-left: 1.5rem; color:#1e3a8a; text-decoration:none; font-weight:600;
}
.top-nav nav a:hover { color:#2563eb; }

.container { max-width: 1200px; margin: 2.5rem auto; padding: 0 1rem; }
.intro h1 { font-size: clamp(1.6rem, 2.2vw, 2.25rem); margin: .25rem 0 .25rem; }
.accent { color: #2563eb; }
.hint { color:#4b5563; margin-top:.25rem; }

/* Grid principal */
.grid {
  display:grid; grid-template-columns: 1fr; gap: 1.5rem; margin-top: 1.25rem;
}
...
.select-wrap select{
  ...
  color:#111;
  border:none; width:100%; padding: .9rem 2.5rem .9rem 1rem;
...
.cards { margin-top: .75rem; display:flex; flex-direction:column; gap:.5rem; }
.card {
  text-align:left; border:none; border-radius:12px;
  background:#1f2937; color:#e5e7eb; padding: .9rem 1rem;
  font-weight:600; cursor:pointer; transition: transform .12s ease, background .2s ease;
}
.card:hover{ transform: translateY(-1px); background:#243041; }
.card.active{ outline:2px solid #2563eb; background:#22314a; }
.card.disabled{ opacity:.45; cursor:not-allowed; }

/* Panel derecho */
.panel {
  display:grid; grid-template-columns: 1fr; background:#111827; border-radius:18px; overflow:hidden;
  box-shadow: 0 10px 20px rgba(0,0,0,.08);
}
@media (min-width: 900px) { .panel { grid-template-columns: 1.25fr .9fr; } }
.panel-content { padding: 1.5rem; color:#f3f4f6; }
.panel-content h3 { margin:0 0 1rem; font-size:1.1rem; font-weight:700; }
...
.field small { color:#9ca3af; display:block; margin-top:.35rem; }

.file {
  display:block; width:100%; padding:.65rem .7rem; border-radius:10px;
...
.btn {
  margin-top: 1rem;
  background:#51c638; color:#fff; border:none; font-weight:800;
  padding:.9rem 1.2rem; border-radius:12px; cursor:pointer;
  box-shadow: 0 6px 0 #3b9a2b; transition: transform .1s ease, filter .2s ease;
}
.btn:hover{ filter:brightness(.98); transform: translateY(-1px); }
.btn:disabled{ background:#9ca3af; box-shadow:none; cursor:not-allowed; }

.files-list { margin-top:1rem; padding-left:0; list-style:none; }
.files-list li { display:flex; align-items:center; gap:.5rem; margin:.35rem 0; color:#d1d5db; }
.files-list li em { color:#9ca3af; font-style:normal; }
.files-list .dot{ width:8px; height:8px; border-radius:999px; background:#2563eb; display:inline-block; }

.note { color:#6b7280; margin-top:.75rem; font-size:.95rem; }
